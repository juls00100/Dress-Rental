...
public class customers extends javax.swing.JFrame {
    ...
    // Use DISTINCT so you don't list the same person multiple times
    String query = "SELECT DISTINCT r_cust_name AS 'Customer Name', r_cust_contact AS 'Contact Number' FROM tbl_rentals";
    
    try (java.sql.Connection conn = config.connectDB();
         java.sql.PreparedStatement pst = conn.prepareStatement(query);
         java.sql.ResultSet rs = pst.executeQuery()) {
        
        // This line fills your JTable automatically
        displyCus.setModel(net.proteanit.sql.DbUtils.resultSetToTableModel(rs));
    ...
    private void initComponents() {
        ...
        displyCus.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jPanel2.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(290, 110, 340, 220));
    ...
    if (row == -1) {
        JOptionPane.showMessageDialog(null, "Please select a customer from the table first!");
        return;
    }

    String oldName = displyCus.getValueAt(row, 0).toString();
    String newName = JOptionPane.showInputDialog(this, "Enter New Name:", oldName);
    String newContact = JOptionPane.showInputDialog(this, "Enter New Contact Number:", displyCus.getValueAt(row, 1).toString());

    if (newName != null && newContact != null) {
        String sql = "UPDATE tbl_rentals SET r_cust_name = ?, r_cust_contact = ? WHERE r_cust_name = ?";
        try (java.sql.Connection conn = config.connectDB();
    ...
    String name = displyCus.getValueAt(row, 0).toString();
    int confirm = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete all rental records for " + name + "?", "Delete Warning", JOptionPane.YES_NO_OPTION);

    if (confirm == JOptionPane.YES_OPTION) {
        String sql = "DELETE FROM tbl_rentals WHERE r_cust_name = ?";
        try (java.sql.Connection conn = config.connectDB();
             java.sql.PreparedStatement pst = conn.prepareStatement(sql)) {
            ...
            JOptionPane.showMessageDialog(null, "Customer Records Deleted!");
    ...
    // We search only the customer-related columns in tbl_rentals
    // We use DISTINCT so a customer doesn't appear multiple times while searching
    String sql = "SELECT DISTINCT r_cust_name AS 'Customer Name', r_cust_contact AS 'Contact Number' "
               + "FROM tbl_rentals WHERE "
               + "r_cust_name LIKE '%" + text + "%' OR "
               + "r_cust_contact LIKE '%" + text + "%'";

    try {
        config conf = new config();
        // Uses the getData method from your config.java
        ...
        if (rs != null) {
            ...
            // Re-apply your centralized styling so it stays pretty!
            config.styleTable(displyCus);
        }
    } catch (Exception e) {
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        String searchText = search_bar.getText().trim();
            String query = "SELECT DISTINCT r_cust_name AS 'Customer Name', r_cust_contact AS 'Contact Number' " +
                           "FROM tbl_rentals WHERE r_cust_name LIKE ? OR r_cust_contact LIKE ?";
            ...
            try (java.sql.Connection conn = config.connectDB();
                ...
                java.sql.ResultSet rs = pst.executeQuery();
                displyCus.setModel(net.proteanit.sql.DbUtils.resultSetToTableModel(rs));
                config.styleTable(displyCus); // Keep your centralized design!
